...
#ifndef MMROS__TENSORRT__TENSORRT_COMMON_HPP_
#define MMROS__TENSORRT__TENSORRT_COMMON_HPP_

#include "mmros/tensorrt/logger.hpp"
...
public:
  ...
  /**
   * @brief Construct a new Trt Common object.
   *
   * @param config TrtCommonConfig object.
   * @param profiler Per-layer profiler.
   * @param plugin_paths Paths for TensorRT plugins.
   */
  TrtCommon(
    const TrtCommonConfig & config,
   ...
   */
  [[nodiscard]] std::string getPrecision() const noexcept;

  /**
   * @brief Get tensor name by index from TensorRT engine with fallback from
   * TensorRT network.
   *
   * @param[in] index Tensor index.
   * @return Tensor name.
   */
  [[nodiscard]] const char * getIOTensorName(const int32_t index) const noexcept;

  /**
   * @brief Get number of IO tensors from TensorRT engine with fallback from
   * TensorRT network.
   *
   * @return Number of IO tensors.
   */
  [[nodiscard]] int32_t getNbIOTensors() const noexcept;

  /**
   * @brief Get tensor shape by index from TensorRT engine with fallback from
   * TensorRT network.
   *
   * @param[in] index Tensor index.
   * @return Tensor shape.
   */
  [[nodiscard]] nvinfer1::Dims getTensorShape(const int32_t index) const noexcept;
   ...
   * @return Tensor shape.
   */
  [[nodiscard]] nvinfer1::Dims getTensorShape(const char * tensor_name) const noexcept;
   ...
   * @return Tensor shape.
   */
  [[nodiscard]] nvinfer1::Dims getInputDims(const int32_t index) const noexcept;
   ...
   * @return Tensor shape.
   */
  [[nodiscard]] nvinfer1::Dims getOutputDims(const int32_t index) const noexcept;
   ...
   */
  bool setTensorAddress(const int32_t index, void * data) noexcept;
   ...
   * @return Whether setting tensor address is successful.
   */
  bool setTensorAddress(const char * tensor_name, void * data) noexcept;
   ...
   */
  bool setTensorsAddresses(std::vector<void *> & tensors) noexcept;
   ...
   * @return Whether setting tensors addresses is successful.
   */
  bool setTensorsAddresses(std::unordered_map<const char *, void *> & tensors) noexcept;
   ...
   */
  bool setInputShape(const int32_t index, const nvinfer1::Dims & dimensions) noexcept;
   ...
   * @return Whether setting input shape is successful.
   */
  bool setInputShape(const char * tensor_name, const nvinfer1::Dims & dimensions) noexcept;

  /**
   * @brief Set inputs shapes by indices via TensorRT context.
   *
   * @param[in] dimensions Vector of tensor dimensions with corresponding
   * indices.
   * @return Whether setting input shapes is successful.
   */
  bool setInputsShapes(const std::vector<nvinfer1::Dims> & dimensions) noexcept;

  /**
   * @brief Set inputs shapes by names via TensorRT context.
   *
   * @param[in] dimensions Map of tensor dimensions with corresponding names as
   * keys.
   * @return Whether setting input shapes is successful.
   */
  bool setInputsShapes(
   ...
   */
  bool setTensor(const int32_t index, void * data, nvinfer1::Dims dimensions = {}) noexcept;
   ...
   * @return Whether setting tensor is successful.
   */
  bool setTensor(const char * tensor_name, void * data, nvinfer1::Dims dimensions = {}) noexcept;

  /**
   * @brief Set tensors (addresses and shapes) by indices via TensorRT context.
   *
   * @param[in] tensors Vector of tensor pointers and dimensions with
   * corresponding indices.
   * @return Whether setting tensors is successful.
   */
  bool setTensors(TensorsVec & tensors) noexcept;

  /**
   * @brief Set tensors (addresses and shapes) by names via TensorRT context.
   *
   * @param[in] tensors Map of tensor pointers and dimensions with corresponding
   * names as keys.
   * @return Whether setting tensors is successful.
   */
  bool setTensors(TensorsMap & tensors) noexcept;
   ...
   */
  [[nodiscard]] std::shared_ptr<Profiler> getModelProfiler() noexcept;
   ...
   * @return Per-layer profiler.
   */
  [[nodiscard]] std::shared_ptr<Profiler> getHostProfiler() noexcept;
   ...
   */
  [[nodiscard]] std::shared_ptr<TrtCommonConfig> getTrtCommonConfig() noexcept;
   ...
   */
  [[nodiscard]] std::shared_ptr<nvinfer1::IBuilderConfig> getBuilderConfig() noexcept;
   ...
   */
  [[nodiscard]] std::shared_ptr<nvinfer1::INetworkDefinition> getNetwork() noexcept;
   ...
   */
  void printProfiling() const noexcept;
...
private:
   ...
   */
  std::string getLayerInformation(nvinfer1::LayerInformationFormat format) noexcept;
