...
#ifndef MMROS__TENSORRT__UTILITY_HPP_
#define MMROS__TENSORRT__UTILITY_HPP_
...
struct TrtCommonConfig
{
  ...
  const fs::path onnx_path;         //!< ONNX model path.
  const std::string precision;      //!< Precision for inference.
  fs::path engine_path;             //!< TensorRT engine path.
  const size_t max_workspace_size;  //!< TensorRT max workspace size.
  const int32_t dla_core_id;        //!< DLA core ID that the process uses.
  const bool profile_per_layer;     //!< Flag for per-layer profiler using IProfiler.
};
...
struct TensorInfo
{
  ...
  int32_t tensor_index;     //!< Tensor index.
};
...
struct NetworkIO : public TensorInfo
{
  ...
  nvinfer1::Dims dims;  //!< Tensor dimensions.
};

/**
 * @brief Represents a tensor optimization profile with minimum, optimal, and
 * maximum dimensions.
 *
 * Example usage:
 * @code
 ...
 * ProfileDims entry_2("tensor_name", {3, {1, 2, 3}}, {3, {1, 3, 4}}, {3, {1, 4,
 * 5}}); bool is_equal = entry_1 == entry_2;  // true
 * @endcode
 */
struct ProfileDims : public TensorInfo
{
  ...
  nvinfer1::Dims min_dims;  //!< Minimum dimensions for optimization.
  nvinfer1::Dims opt_dims;  //!< Optimal dimensions for optimization.
  nvinfer1::Dims max_dims;  //!< Maximum dimensions for optimization.
};

//!< @brief Valid calibration types for TensorRT.
constexpr std::array<std::string_view, 4> VALID_CALIBRATIONS = {
...
struct CalibrationConfig
{
   ...
   * @param[in] quantize_first_layer Flag for partial quantization in first
   * layer.
  ...
  {
    if (
      std::find(VALID_CALIBRATIONS.begin(), VALID_CALIBRATIONS.end(), calib_type_str) ==
      VALID_CALIBRATIONS.end()) {
  ...
  const std::string calib_type_str;  //!< Type of calibration
  const bool quantize_first_layer;   //!< Flag for partial quantization in
                                     //!< first layer
  const bool quantize_last_layer;    //!< Flag for partial quantization in
                                     //!< last layer
