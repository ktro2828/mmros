...
struct nmsOutLaunchConfig
{
  ...
  nmsOutLaunchConfig(nvinfer1::DataType t_bbox, nvinfer1::DataType t_score, bool rotated)
  : t_bbox(t_bbox), t_score(t_score), rotated(rotated)
  {
  }
  nmsOutLaunchConfig(
    nvinfer1::DataType t_bbox, nvinfer1::DataType t_score, bool rotated, nmsOutFunc function)
...
bool nmsOutputInit()
{
  nmsOutFuncVec.push_back(nmsOutLaunchConfig(
    nvinfer1::DataType::kFLOAT, nvinfer1::DataType::kFLOAT, false,
    gatherNMSOutputs_gpu<float, float, false>));
  nmsOutFuncVec.push_back(nmsOutLaunchConfig(
    nvinfer1::DataType::kFLOAT, nvinfer1::DataType::kFLOAT, true,
...
pluginStatus_t gatherNMSOutputs(
  cudaStream_t stream, const bool shareLocation, const int numImages, const int numPredsPerClass,
  const int numClasses, const int topK, const int keepTopK, const nvinfer1::DataType DT_BBOX,
  const nvinfer1::DataType DT_SCORE, const void * indices, const void * scores,
  const void * bboxData, void * nmsedDets, void * nmsedLabels, void * nmsedIndex, bool clipBoxes,
  bool rotated)
{
