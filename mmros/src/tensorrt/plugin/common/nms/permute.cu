...
{
  nvinfer1::DataType t_data;
  pdFunc function;

  pdLaunchConfig(nvinfer1::DataType t_data) : t_data(t_data) {}
  pdLaunchConfig(nvinfer1::DataType t_data, pdFunc function) : t_data(t_data), function(function) {}
...
{
  pdFuncVec.push_back(pdLaunchConfig(nvinfer1::DataType::kFLOAT, permuteData_gpu<float>));
  return true;
}
...
pluginStatus_t permuteData(
  cudaStream_t stream, const int nthreads, const int num_classes, const int num_data,
  const int num_dim, const nvinfer1::DataType DT_DATA, bool confSigmoid, const void * data,
  void * new_data)
{
