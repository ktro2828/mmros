...
struct sspcLaunchConfig
{
  ...
  sspcLaunchConfig(nvinfer1::DataType t_score) : t_score(t_score) {}
  sspcLaunchConfig(nvinfer1::DataType t_score, sspcFunc function)
...
bool sspcInit()
{
  sspcFuncVec.push_back(
    sspcLaunchConfig(nvinfer1::DataType::kFLOAT, sortScoresPerClass_gpu<float>));
  return true;
}
...
pluginStatus_t sortScoresPerClass(
  ...
  const nvinfer1::DataType DT_SCORE, void * conf_scores_gpu, void * index_array_gpu,
  void * workspace)
{
...
size_t sortScoresPerClassWorkspaceSize(
  const int num, const int num_classes, const int num_preds_per_class,
  const nvinfer1::DataType DT_CONF)
{
  size_t wss[4];
  const int arrayLen = num * num_classes * num_preds_per_class;
  wss[0] = arrayLen * getElementSize(DT_CONF);     // temp scores
  wss[1] = arrayLen * sizeof(int);                 // temp indices
  wss[2] = (num * num_classes + 1) * sizeof(int);  // offsets
  if (DT_CONF == nvinfer1::DataType::kFLOAT) {
