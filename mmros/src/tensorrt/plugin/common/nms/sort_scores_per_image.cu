...
#include "mmros/tensorrt/plugin/common/nms/cub_helper.hpp"
...
struct sspiLaunchConfig
{
  ...
  sspiLaunchConfig(nvinfer1::DataType t_score) : t_score(t_score) {}
  sspiLaunchConfig(nvinfer1::DataType t_score, sspiFunc function)
...
bool sspiInit()
{
  sspiFuncVec.push_back(
    sspiLaunchConfig(nvinfer1::DataType::kFLOAT, sortScoresPerImage_gpu<float>));
  return true;
}
...
pluginStatus_t sortScoresPerImage(
  ...
  const nvinfer1::DataType DT_SCORE, void * unsorted_scores, void * unsorted_bbox_indices,
...
size_t sortScoresPerImageWorkspaceSize(
  const int num_images, const int num_items_per_image, const nvinfer1::DataType DT_SCORE)
{
  const int arrayLen = num_images * num_items_per_image;
  size_t wss[2];
  wss[0] = (num_images + 1) * sizeof(int);  // offsets
  if (DT_SCORE == nvinfer1::DataType::kFLOAT) {
