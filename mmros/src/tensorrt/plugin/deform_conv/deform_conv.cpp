...
#include "mmros/tensorrt/plugin/deform_conv/deform_conv.hpp"
  ...
  const nvinfer1::PluginTensorDesc * outputs, int nbOutputs) const TRT_NOEXCEPT
{
  ...
  size_t col_size =
    getAlignedSize(nInputPlane * kW * kH * im2col_step * outputHeight * outputWidth * sizeof_dtype);

  size_t out_size = 0;
  if (im2col_step != 1)
    out_size =
      getAlignedSize(batch_size * nOutputPlane * outputHeight * outputWidth * sizeof_dtype);
  ...
  cudaStream_t stream) TRT_NOEXCEPT
{
  ...
  switch (data_type) {
    case nvinfer1::DataType::kFLOAT:
      deform_conv<float>(
        (float *)x, (float *)weight, (float *)offset, (float *)output, workSpace, batch, channels,
        height, width, channels_out, kernel_w, kernel_h, mStride.d[0], mStride.d[1], mPadding.d[0],
        mPadding.d[1], mDilation.d[0], mDilation.d[1], mGroup, mDeformableGroup, im2col_step,
        m_cublas_handle, stream);
      break;
    case nvinfer1::DataType::kHALF:
      deform_conv<half>(
        (half *)x, (half *)weight, (half *)offset, (half *)output, workSpace, batch, channels,
        height, width, channels_out, kernel_w, kernel_h, mStride.d[0], mStride.d[1], mPadding.d[0],
        mPadding.d[1], mDilation.d[0], mDilation.d[1], mGroup, mDeformableGroup, im2col_step,
        m_cublas_handle, stream);
      break;
...
nvinfer1::IPluginV2DynamicExt * DeformableConvPluginDynamicCreator::createPlugin(
...
nvinfer1::IPluginV2DynamicExt * DeformableConvPluginDynamicCreator::deserializePlugin(
