...
#include "mmros/tensorrt/plugin/modulated_deform_conv/modulated_deform_conv_cuda.cuh"
#include "mmros/tensorrt/plugin/modulated_deform_conv/modulated_deform_conv_kernel.hpp"
...
void trt_modulated_deformable_im2col(
  const T * data_im_, const T * data_offset_, const T * data_mask_, const int batch_size,
  const int channels, const int height_im, const int width_im, const int height_col,
  const int width_col, const int kernel_h, const int kenerl_w, const int pad_h, const int pad_w,
  const int stride_h, const int stride_w, const int dilation_h, const int dilation_w,
  const int deformable_group, T * data_col_, cudaStream_t stream)
{
...
template <>
__global__ void output_add_bias_kernel<__half>(
