...
#include "mmros/tensorrt/plugin/multi_level_roi_align/multi_level_roi_align.hpp"
...
nvinfer1::IPluginV2DynamicExt * TRTMultiLevelRoiAlign::clone() const TRT_NOEXCEPT
{
  TRTMultiLevelRoiAlign * plugin = new TRTMultiLevelRoiAlign(
    mLayerName, mAlignedHeight, mAlignedWidth, mPoolMode, mSampleNum, mFeatmapStrides,
...
int TRTMultiLevelRoiAlign::enqueue(
  ...
  cudaStream_t stream) TRT_NOEXCEPT
{
  ...
  multi_level_roi_align<float>(
    (float *)outputs[0], (const float *)rois, num_rois, feats, num_feats, batch_size, channels,
    &heights[0], &widths[0], &strides[0], mAlignedHeight, mAlignedWidth, mPoolMode, mSampleNum,
...
nvinfer1::IPluginV2DynamicExt * TRTMultiLevelRoiAlignCreator::createPlugin(
  const char * name, const nvinfer1::PluginFieldCollection * fc) TRT_NOEXCEPT
{
  ...
  TRTMultiLevelRoiAlign * plugin = new TRTMultiLevelRoiAlign(
    name, alignedHeight, alignedWidth, poolMode, sampleNum, featmapStrides, roiScaleFactor,
    finestScale, aligned);
  plugin->setPluginNamespace(getPluginNamespace());
  return plugin;
}

nvinfer1::IPluginV2DynamicExt * TRTMultiLevelRoiAlignCreator::deserializePlugin(
