...
bool TrtCommon::setup(ProfileDimsPtr profile_dims, NetworkIOPtr network_io)
{
  ...
  auto build_engine_with_log = [this]() -> bool {
    ...
    auto log_thread = logger_->log_throttle(
      ...
      "Applying optimizations and building TensorRT "
      "CUDA engine. Please wait for a few minutes...",
      5);
  ...
  if (fs::exists(trt_config_->engine_path)) {
    ...
    if (!validateNetworkIO() || !validateProfileDims()) {
      logger_->log(
        ...
        "Network validation failed for loaded engine from file. "
        "Rebuilding engine");
  ...
  if (!validateNetworkIO() || !validateProfileDims()) {
    logger_->log(
      ...
      "deployed model has been changed. Check your configuration file with "
      "the current model.");
    return false;
  }
...
std::string TrtCommon::getPrecision() const noexcept
{
  return trt_config_->precision;
}

const char * TrtCommon::getIOTensorName(const int32_t index) const noexcept
{
  if (!engine_) {
    logger_->log(
      nvinfer1::ILogger::Severity::kWARNING,
...
int32_t TrtCommon::getNbIOTensors() const noexcept
{
  if (!engine_) {
    logger_->log(
      nvinfer1::ILogger::Severity::kWARNING,
...
nvinfer1::Dims TrtCommon::getTensorShape(const int32_t index) const noexcept
{
  if (!engine_) {
    logger_->log(
      nvinfer1::ILogger::Severity::kWARNING,
...
nvinfer1::Dims TrtCommon::getTensorShape(const char * tensor_name) const noexcept
{
  if (!engine_) {
...
nvinfer1::Dims TrtCommon::getInputDims(const int32_t index) const noexcept
{
  if (!network_) {
    logger_->log(nvinfer1::ILogger::Severity::kERROR, "Network is not initialized");
    return {};
...
nvinfer1::Dims TrtCommon::getOutputDims(const int32_t index) const noexcept
{
  if (!network_) {
    logger_->log(nvinfer1::ILogger::Severity::kERROR, "Network is not initialized");
    return {};
...
bool TrtCommon::setTensorAddress(const int32_t index, void * data) noexcept
{
  auto const & name = getIOTensorName(index);
  return setTensorAddress(name, data);
}

bool TrtCommon::setTensorAddress(const char * tensor_name, void * data) noexcept
{
  if (!context_) {
...
bool TrtCommon::setTensorsAddresses(std::vector<void *> & tensors) noexcept
{
  bool success = true;
...
bool TrtCommon::setTensorsAddresses(std::unordered_map<const char *, void *> & tensors) noexcept
{
  bool success = true;
...
bool TrtCommon::setInputShape(const int32_t index, const nvinfer1::Dims & dimensions) noexcept
{
  auto const & name = getIOTensorName(index);
  return setInputShape(name, dimensions);
}

bool TrtCommon::setInputShape(const char * tensor_name, const nvinfer1::Dims & dimensions) noexcept
{
  if (!context_) {
    logger_->log(nvinfer1::ILogger::Severity::kERROR, "Context is not initialized");
    return false;
...
bool TrtCommon::setInputsShapes(const std::vector<nvinfer1::Dims> & dimensions) noexcept
{
  bool success = true;
...
bool TrtCommon::setInputsShapes(
  const std::unordered_map<const char *, nvinfer1::Dims> & dimensions) noexcept
{
  bool success = true;
...
bool TrtCommon::setTensor(const int32_t index, void * data, nvinfer1::Dims dimensions) noexcept
{
...
bool TrtCommon::setTensor(const char * tensor_name, void * data, nvinfer1::Dims dimensions) noexcept
{
...
bool TrtCommon::setTensors(TensorsVec & tensors) noexcept
{
  bool success = true;
  for (std::size_t i = 0, e = tensors.size(); i < e; i++) {
...
bool TrtCommon::setTensors(TensorsMap & tensors) noexcept
{
  bool success = true;
  for (auto const & tensor : tensors) {
...
std::shared_ptr<Profiler> TrtCommon::getModelProfiler() noexcept
{
  return model_profiler_;
}

std::shared_ptr<Profiler> TrtCommon::getHostProfiler() noexcept
{
  return host_profiler_;
}

std::shared_ptr<TrtCommonConfig> TrtCommon::getTrtCommonConfig() noexcept
{
  return trt_config_;
}

std::shared_ptr<nvinfer1::IBuilderConfig> TrtCommon::getBuilderConfig() noexcept
{
  return builder_config_;
}

std::shared_ptr<nvinfer1::INetworkDefinition> TrtCommon::getNetwork() noexcept
{
  return network_;
}

std::shared_ptr<Logger> TrtCommon::getLogger() noexcept
{
  return logger_;
}

bool TrtCommon::enqueueV3(cudaStream_t stream) noexcept
{
  if (!context_) {
    logger_->log(nvinfer1::ILogger::Severity::kERROR, "Context is not initialized");
    return false;
...
void TrtCommon::printProfiling() const noexcept
{
  logger_->log(
...
std::string TrtCommon::getLayerInformation(nvinfer1::LayerInformationFormat format) noexcept
{
  if (!context_ || !engine_) {
...
bool TrtCommon::buildEngineFromOnnx()
{
  ...
  file.write(reinterpret_cast<const char *>(plan->data()),
             plan->size());  // NOLINT
  file.close();
...
bool TrtCommon::validateProfileDims()
{
  ...
  if (!profile_dims_ || profile_dims_->empty()) {
    logger_->log(
      nvinfer1::ILogger::Severity::kWARNING,
      "Input profile is empty, skipping validation. If network has "
      "dynamic shapes, it might lead "
  ...
  if (engine_->getNbOptimizationProfiles() != 1) {
    logger_->log(
      nvinfer1::ILogger::Severity::kWARNING,
      "Number of optimization profiles in the engine (%d) is not "
      "equal to 1. Selecting the first "
      "cached profile.",
...
bool TrtCommon::validateNetworkIO()
{
  ...
  if (!network_io_ || network_io_->empty()) {
    logger_->log(
      nvinfer1::ILogger::Severity::kWARNING,
      "Network IO is empty, skipping validation. It might lead to "
      "undefined behavior");
    return success;
  }
  ...
  if (network_io_->size() != static_cast<size_t>(getNbIOTensors())) {
    ...
    logger_->log(
      nvinfer1::ILogger::Severity::kWARNING,
      "Number of tensors in the engine (%d) does not match number "
      "of tensors in the config (%d). "
